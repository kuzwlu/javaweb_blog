...
public class KuzwluServlet extends HttpServlet {
	...
	throws ServletException, IOException {
		...
		for(Article p :articles){
			...
			String manageruser =p.getAruser();
			...
			blog+="<div class='main-post post-box card'>" +
					"<div class='subimg'><a href='#'><img src='"+image+"' onerror='whenError(this)'' /></a></div>" +
							"<div class='title'><a onclick='getimage("+arid+")' >"+title+"</a></div>" +
									"<div class='main-content'><span>"+html+"...</span></div>" +
									"<div class='main-more'><span>作者:<a onclick='getimage("+arid+")'>"+manageruser+"</a>	</span>" +
									"<span>时间:<a href='#'>"+time+"</a>	</span>" +
									"<span>评论:<a onclick='getimage("+arid+")'>"+thenumber+"</a>	</span>" +
									"<span>类别:<a href='#'>"+caname+"</a>	</span>" +
									"<span>浏览:<a href='#'>"+p.getClick()+"人</a>	</span></div></div>";
...
//		request.getRequestDispatcher(path).forward(request, response);
	...
	throws ServletException, IOException {
		...
		String nextpage="<a onclick='nextpage("+maxpage+")'style='color: #7ea5de;''>点击加载更多</a>";
		CatalogDAO catalogDAO = new CatalogDAO();
		ArrayList<Catalog> catalogs = catalogDAO.finabystate(castate);
		ArrayList<Article> articlecounts = articleDAO.finaAll();
